...
export default function BotonVerificar({ perfilId }: { perfilId: string }) {
  ...
  if (rechazando) {
    return (
      <div className="mt-3 rounded-xl border border-alerta/30 bg-alerta-suave p-3">
        ...
        <p className="mt-0.5 text-[11.5px] leading-snug text-tinta-2">
        ...
        <textarea
          ...
          className="mt-2 w-full resize-none rounded-lg border border-linea bg-white px-3 py-2 text-[13px] leading-relaxed text-tinta outline-none placeholder:text-tinta-3 focus:border-marca-2"
        />

        {error && (
          <p className="mt-1.5 text-[11.5px] text-alerta">{error}</p>
        )}
        ...
        <div className="mt-2 flex gap-2">
          <button
            ...
            disabled={enviando}
            className="flex-1 rounded-lg bg-alerta py-2.5 text-[13px] font-bold text-white disabled:opacity-60"
          >
          ...
          <button
            ...
            className="rounded-lg border border-linea bg-white px-3 py-2.5 text-[13px] font-semibold text-tinta-2"
          >
            Volver
          </button>
        </div>
  ...
  return (
    <div className="mt-3">
      {error && (
        <p className="mb-2 rounded-lg border border-alerta/30 bg-alerta-suave px-3 py-2 text-[12px] text-tinta-2">
          {error}
        </p>
      )}
      <div className="flex gap-2">
        <button
          ...
          disabled={enviando}
          className="flex-1 rounded-lg bg-ok py-2.5 text-[13.5px] font-bold text-white disabled:opacity-60"
        >
        ...
        <button
          ...
          disabled={enviando}
          className="rounded-lg border border-linea bg-white px-4 py-2.5 text-[13.5px] font-semibold text-tinta-2"
        >
          Rechazar
        </button>
      </div>
