...
export default async function PerfilPublico({
  ...
  const color = COLORES[pro.nombre.length % COLORES.length];
    ...
    <main className="mx-auto min-h-screen max-w-md bg-white pb-12">
      <header className="bg-marca px-5 pt-6 pb-6 text-white">
        ...
        <div className="mt-4 flex items-center gap-3.5">
          <span
            className="font-display grid size-16 shrink-0 place-items-center rounded-2xl text-[21px] font-bold"
            style={{ background: color }}
          >
            {iniciales}
          </span>
          <div className="min-w-0">
            <h1 className="font-display text-[20px] leading-tight font-extrabold">
              {pro.nombre}
            </h1>
      ...
      {pro.bio && (
        <div className="border-b border-linea px-5 py-4">
          <p className="text-[13.5px] leading-relaxed text-tinta-2">{pro.bio}</p>
        </div>
      )}
      ...
      <div className="border-b border-linea px-5 py-4">
        <h2 className="font-display mb-3 text-[14.5px] font-bold text-tinta">
          Qué verificamos
        </h2>
        ...
        <p className="mt-3 text-[11px] leading-snug text-tinta-3">
      ...
      {pro.zonas?.length > 0 && (
        <div className="border-b border-linea px-5 py-4">
          <h2 className="font-display mb-2.5 text-[14.5px] font-bold text-tinta">
            Dónde trabaja
          </h2>
          <div className="flex flex-wrap gap-1.5">
            {pro.zonas.map((z: string) => (
              <span
                key={z}
                className="rounded-md border border-linea-2 bg-fondo px-2 py-1 text-[11px] font-semibold text-tinta-2"
              >
                {z}
              </span>
            ))}
            <span className="rounded-md border border-linea-2 bg-fondo px-2 py-1 text-[11px] font-semibold text-tinta-2">
      ...
      <div className="px-5 py-4">
        ...
        {resenas && resenas.length > 0 ? (
            ...
            {resenas.map((r) => (
              <div key={r.id} className="border-b border-linea-2 pb-3.5 last:border-0">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <span className="text-[13px] text-acento">
                    {"★".repeat(r.estrellas)}
                    <span className="text-linea">
                      {"★".repeat(5 - r.estrellas)}
                    </span>
                  </span>
                  <time className="text-[11px] text-tinta-3">
          ...
          <p className="rounded-2xl border border-dashed border-linea px-4 py-6 text-center text-[13px] leading-relaxed text-tinta-3">
...
function Numero({
  ...
  return (
    ...
    >
      <b className="font-display block text-[18px] font-bold text-tinta tabular-nums">
        {valor}
      </b>
      <span className="mt-0.5 block text-[10.5px] text-tinta-3">{texto}</span>
    </div>
  );
}
...
function Verificacion({
  ...
  return (
    <div className="flex items-start gap-2.5">
      ...
      <div>
        <b className="block text-[13px] font-semibold text-tinta">{titulo}</b>
        <span className="block text-[11.5px] text-tinta-3">{detalle}</span>
      </div>
    </div>
  );
}
